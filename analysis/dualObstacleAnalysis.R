...
obs_dual_train <- obsdual %>%
  filter(condition == 'train_dual') # looking only at training

taskmeans <- obs_dual_train %>%
  group_by(subject) %>%
  group_by(trial) %>% 
...
ggplot(data = taskmeans, aes(x = trial, y = Mean_RMSExy)) +
  ...
  ylab("Mean RMSExy") +
  xlab("Trial") +
  ggtitle('Dual Obstacle: Training with Obstructed and Unobstructed Targets')

## stats

block1 <- obs_dual_train %>%
  group_by(subject) %>% 
  filter(trial %in% min(trial)) %>%
  summarise(RMSE = mean(RMSExy, na.rm = TRUE), block = 1)

blocklast <- obs_dual_train %>%
  group_by(subject) %>%
...
# ANALYZE LEARNING FOR EACH OBSTACLE AND ONLY "FREE" TARGET SET
...
unobs_df <- rbind(obs_dual_train_CCW, obs_dual_train_CW) # only unobstructed targets per rotation/obstacle
...
for (rot in sort(unique(unobs_df$rotationval))){
  ...
  ppdf3 <- unobs_df %>%
    filter(rotationval == rot) %>%
    group_by(subject) %>%
    filter(trialct %in% c(max(trialct)-2, max(trialct)-1, max(trialct))) %>%
